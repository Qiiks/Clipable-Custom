...
const MaxStorageBytes = 5 * GB // 5GB storage limit
...
func (s *store) GetTotalStorageUsed(ctx context.Context) (int64, error) {
	var totalSize int64
	objectsCh := s.s3.ListObjects(ctx, s.cfg.S3.Bucket, minio.ListObjectsOptions{Recursive: true})
...
func (s *store) cleanupOldClips(ctx context.Context) error {
	totalSize, err := s.GetTotalStorageUsed(ctx)
	if err != nil {
		return err
	}
	...
	// List all objects sorted by last modified
	var objects []minio.ObjectInfo
	for obj := range s.s3.ListObjects(ctx, s.cfg.S3.Bucket, minio.ListObjectsOptions{Recursive: true}) {
		if obj.Err != nil {
			continue
		}
	...
	// Delete oldest clips until we're under the limit
	for _, obj := range objects {
		if totalSize < MaxStorageBytes {
			break
		}

		err := s.s3.RemoveObject(ctx, s.cfg.S3.Bucket, obj.Key, minio.RemoveObjectOptions{})
		if err != nil {
			log.WithError(err).Error("Failed to remove old clip")
			continue
		}
...
func (s *store) PutObject(ctx context.Context, cid int64, filename string, r io.Reader) (int64, error) {
	...
	if err := s.cleanupOldClips(ctx); err != nil {
		log.WithError(err).Error("Failed to cleanup old clips")
	}
